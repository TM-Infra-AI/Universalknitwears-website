var accordion= {
	...
	event: function(options){
		var that=this,
				$items=jQuery("#"+options.id).find(".pana-accordion-item");
		$items.on("mouseover",function(){
			if(options.autoPlay){
				that.clearAnimate();
			}
			that.active(options,jQuery(this));
		});
	...
	autoPlay: function(options){
		...
		that.delay=setTimeout(function(){
			$next=jQuery("#"+options.id).find(".active").next();
			if($next.length==0) {
				$next=jQuery("#"+options.id).find(".pana-accordion-item:eq(0)");
			}
	...
	active: function(options,$current){
		...
		var $items=jQuery("#"+options.id).find(".pana-accordion-item"),
		...
		$items.each(function(i,elem){
			var $item=jQuery(elem),
					$next=$current.next().length==0 ? jQuery($items[1]) : $current.next(),
