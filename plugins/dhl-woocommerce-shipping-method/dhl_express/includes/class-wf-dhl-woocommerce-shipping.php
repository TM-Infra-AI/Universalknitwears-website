...
class wf_dhl_woocommerce_shipping_method extends WC_Shipping_Method {
    ...
    border: 1px solid #ccc;border-top: unset !important;padding: 5px;">';
                            require_once('settings/dhl_premium_settings.php');
								echo '</div>';
                        break;
                }
                echo '
       ...
       public function wf_dhl_shipping_page_tabs($current = 'general')
       {
        $tabs = array(
                    ...
                    'premium' => __("<strong style='color:red;'> Go Premium! </strong>", 'wf-shipping-dhl')
                );
    ...
    private function get_dhl_requests($dhl_packages, $package) {
        ...
        $insurance_details = $this->insure_contents ? "<InsuredValue>{$total_value}</InsuredValue><InsuredCurrency>{$woocommerce_currency}</InsuredCurrency>" : "";
