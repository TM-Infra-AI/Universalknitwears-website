...
if ( ! class_exists( 'ADQ_PB' ) ) :
/**
 * ADQ_WPCF7
 */
class ADQ_PB extends WC_PB_Cart {
	...
	public function __construct() {                     
                if( isset( $_POST["is_quote"] ) || StaticAdqQuoteRequest::is_quote_list_page()  ) {
                        $this->is_quote_submit = true;
                }
                
                add_action( 'woocommerce_bundles_add_to_cart_button', array( $this, 'adq_bundles_add_to_cart_button' ), 20 );
                
                add_filter( 'woocommerce_quote_item_class', array( WC_PB()->display, 'woo_bundles_table_item_class' ), 10, 2 );
                
                if( $this->is_quote_submit ) {                            
                        /* OVERRIDE ALL HOOKS */                        
                        remove_filter( 'woocommerce_add_to_cart_validation', array( WC_PB()->cart, 'woo_bundles_validation' ), 10 );
                        remove_filter( 'woocommerce_add_cart_item_data', array( WC_PB()->cart, 'woo_bundles_add_cart_item_data' ), 10 );
                        remove_action( 'woocommerce_add_to_cart', array( WC_PB()->cart, 'woo_bundles_add_bundle_to_cart' ), 10 );
                        remove_filter( 'woocommerce_add_cart_item', array( WC_PB()->cart, 'woo_bundles_add_cart_item_filter' ), 10 );
                        remove_filter( 'woocommerce_get_cart_item_from_session', array( WC_PB()->cart, 'woo_bundles_get_cart_data_from_session' ), 10 );
                        remove_filter( 'woocommerce_cart_item_quantity', array( WC_PB()->cart, 'woo_bundles_cart_item_quantity' ), 10 );
                        remove_filter( 'woocommerce_cart_item_remove_link', array( WC_PB()->cart, 'woo_bundles_cart_item_remove_link' ), 10 );
                        remove_action( 'woocommerce_after_cart_item_quantity_update', array( WC_PB()->cart, 'woo_bundles_update_quantity_in_cart' ), 1 );
                        remove_action( 'woocommerce_before_cart_item_quantity_zero', array( WC_PB()->cart, 'woo_bundles_update_quantity_in_cart' ), 1 );
                        remove_filter( 'woocommerce_order_again_cart_item_data', array( WC_PB()->cart, 'woo_bundles_order_again' ), 10 );
                        remove_filter( 'woocommerce_cart_item_price', array( WC_PB()->cart, 'woo_bundles_cart_item_price_html' ), 10 );
                        remove_filter( 'woocommerce_cart_item_subtotal', array( WC_PB()->cart, 'woo_bundles_item_subtotal' ), 10 );
                        remove_filter( 'woocommerce_checkout_item_subtotal', array( WC_PB()->cart, 'woo_bundles_item_subtotal' ), 10 );
                        remove_action( 'woocommerce_cart_item_removed', array( WC_PB()->cart, 'woo_bundles_cart_item_removed' ), 10 );
                        remove_action( 'woocommerce_cart_item_restored', array( WC_PB()->cart, 'woo_bundles_cart_item_restored' ), 10 );
                        remove_filter( 'woocommerce_cart_shipping_packages', array( WC_PB()->cart, 'woo_bundles_shipping_packages_fix' ) );                
                        ...
                        add_filter( 'woocommerce_get_quote_item_from_session', array( $this, 'woo_bundles_get_cart_data_from_session' ), 10, 3 );

                        // Sync quantities of bundled items with bundle quantity
                        add_filter( 'woocommerce_quote_item_quantity', array( $this, 'woo_bundles_cart_item_quantity' ), 10, 2 );
                        add_filter( 'woocommerce_quote_item_remove_link', array( $this, 'woo_bundles_cart_item_remove_link' ), 10, 2 );
                        ...
                        add_filter( 'woocommerce_quote_item_price', array( $this, 'woo_bundles_cart_item_price_html' ), 10, 3 );

                        // Modify cart items subtotals according to the pricing strategy used (static / per-product)
                        add_filter( 'woocommerce_quote_item_subtotal', array( $this, 'woo_bundles_item_subtotal' ), 10, 3 );
        ...
        public function adq_bundles_add_to_cart_button () {
                global $product;
                
                if( !did_action('woocommerce_bundles_add_to_cart_button') ) {
                        return;
                }
                
                remove_action("woocommerce_after_add_to_cart_button", "adq_single_shortcode", 2);
                remove_action("woocommerce_after_single_variation", "adq_single_variation_shortcode", 2);
                
                echo StaticAdqQuoteRequest::generate_quote_button( $product, 'bundle_add_to_cart_button' );                
        }
	...
	public function woo_bundles_add_bundle_to_cart( $bundle_cart_key, $bundle_id, $bundle_quantity, $variation_id, $variation, $cart_item_data ) {
		...
		if ( isset( $cart_item_data[ 'stamp' ] ) && ! isset( $cart_item_data[ 'bundled_by' ] ) ) {
			...
			// the bundle
			$bundle = WC_Adq()->quote->quote_contents[ $bundle_cart_key ][ 'data' ];
			...
			foreach ( $cart_item_data[ 'stamp' ] as $bundled_item_id => $bundled_item_stamp ) {
				...
				if ( $bundled_item_cart_key && ! in_array( $bundled_item_cart_key, WC_Adq()->quote->quote_contents[ $bundle_cart_key ][ 'bundled_items' ] ) ) {
					WC_Adq()->quote->quote_contents[ $bundle_cart_key ][ 'bundled_items' ][] = $bundled_item_cart_key;
				}
	...
	public function bundled_add_to_cart( $bundle_id, $product_id, $quantity = 1, $variation_id = '', $variation = '', $cart_item_data ) {
		...
		$cart_id = WC_Adq()->quote->generate_quote_id( $product_id, $variation_id, $variation, $cart_item_data );

		// See if this product and its options is already in the cart
		$cart_item_key = WC_Adq()->quote->find_product_in_quote( $cart_id );
		...
		if ( ! $cart_item_key ) {
			...
			WC_Adq()->quote->quote_contents[ $cart_item_key ] = apply_filters( 'woocommerce_add_cart_item', array_merge( $cart_item_data, array(
	...
	public function woo_bundles_add_cart_item_filter( $cart_item, $cart_key ) {
		...
		$quote_contents = WC_Adq()->quote->quote_contents;
		...
		if ( isset( $cart_item[ 'bundled_by' ] ) && isset( $cart_item[ 'bundled_item_id' ] ) ) {
			...
			if ( isset( $quote_contents[ $bundle_cart_key ] ) ) {

				$bundle = $quote_contents[ $bundle_cart_key ][ 'data' ];
	...
	public function woo_bundles_get_cart_data_from_session( $cart_item, $item_session_values, $cart_item_key ) {
		...
		$quote_contents = ! empty( WC_Adq()->quote ) ? WC_Adq()->quote->quote_contents : '';
		...
		if ( isset( $item_session_values[ 'bundled_by' ] ) && isset( $item_session_values[ 'bundled_item_id' ] ) ) {
			...
			if ( isset( $quote_contents[ $bundle_cart_key ] ) && ! empty( $quote_contents[ $bundle_cart_key ][ 'bundled_items' ] ) ) {
	...
	public function woo_bundles_cart_item_remove_link( $link, $cart_item_key ) {
		...
		if ( isset( WC_Adq()->quote->quote_contents[ $cart_item_key ][ 'bundled_by' ] ) ) {

			$bundle_cart_key = WC_Adq()->quote->quote_contents[ $cart_item_key ][ 'bundled_by' ];

			if ( isset( WC_Adq()->quote->quote_contents[ $bundle_cart_key ] ) ) {
				return '';
			}
		}
	...
	public function woo_bundles_cart_item_quantity( $quantity, $cart_item_key ) {
		...
		if ( isset( WC_Adq()->quote->quote_contents[ $cart_item_key ][ 'stamp' ] ) ) {

			if ( isset( WC_Adq()->quote->quote_contents[ $cart_item_key ][ 'bundled_by' ] ) ) {
				return WC_Adq()->quote->quote_contents[ $cart_item_key ][ 'quantity' ];
			}
		}
	...
	public function woo_bundles_update_quantity_in_cart( $cart_item_key, $quantity = 0 ) {
		...
		if ( ! empty( WC_Adq()->quote->quote_contents[ $cart_item_key ] ) ) {

			if ( $quantity == 0 || $quantity < 0 ) {
				$quantity = 0;
			} else {
				$quantity = WC_Adq()->quote->quote_contents[ $cart_item_key ][ 'quantity' ];
			}

			if ( ! empty( WC_Adq()->quote->quote_contents[ $cart_item_key ][ 'stamp' ] ) && ! isset( WC_Adq()->quote->quote_contents[ $cart_item_key ][ 'bundled_by' ] ) ) {

				// unique bundle stamp added to all bundled items & the grouping item
				$stamp = WC_Adq()->quote->quote_contents[ $cart_item_key ][ 'stamp' ];

				// change the quantity of all bundled items that belong to the same bundle config
				foreach ( WC_Adq()->quote->quote_contents as $key => $value ) {
					...
					if ( isset( $value[ 'bundled_by' ] ) && isset( $value[ 'stamp' ] ) && $cart_item_key == $value[ 'bundled_by' ] && $stamp == $value[ 'stamp' ] ) {
						...
						if ( $value[ 'data' ]->is_sold_individually() && $quantity > 0 ) {
							WC_Adq()->quote->set_quantity( $key, 1, false );
						} else {
							$bundle_quantity = $value[ 'stamp' ][ $value[ 'bundled_item_id' ] ][ 'quantity' ];
							WC_Adq()->quote->set_quantity( $key, $quantity * $bundle_quantity, false );
						}
					}
				}
			}
	...
	public function woo_bundles_cart_item_price_html( $price, $values, $cart_item_key ) {
		...
		if ( isset( $values[ 'bundled_by' ] ) ) {
			...
			if ( isset( WC_Adq()->quote->quote_contents[ $bundle_cart_key ] ) ) {
				...
				if ( ( WC_Adq()->quote->quote_contents[ $bundle_cart_key ][ 'data' ]->is_priced_per_product() == false && $values[ 'data' ]->price == 0 ) || isset( WC_Adq()->quote->quote_contents[ $bundle_cart_key ][ 'composite_parent' ] ) && $values[ 'data' ]->price == 0 ) {
					return '';
				}
			}
		}
	...
	public function woo_bundles_item_subtotal( $subtotal, $values, $cart_item_key ) {
		...
		if ( isset( $values[ 'bundled_by' ] ) ) {
			...
			if ( isset( WC_Adq()->quote->quote_contents[ $bundle_cart_key ] ) ) {
				...
				if ( ( false === WC_Adq()->quote->quote_contents[ $bundle_cart_key ][ 'data' ]->is_priced_per_product() ) || isset( WC_Adq()->quote->quote_contents[ $bundle_cart_key ][ 'composite_parent' ] ) ) {
					return '';
				} else {
		...
		if ( isset( $values[ 'bundled_items' ] ) ) {
			...
			foreach ( $values[ 'bundled_items' ] as $bundled_item_key ) {
				...
				if ( ! isset( WC_Adq()->quote->quote_contents[ $bundled_item_key ] ) ) {
					continue;
				}

				$item_values        = WC_Adq()->quote->quote_contents[ $bundled_item_key ];
	...
	public function woo_bundles_cart_item_removed( $cart_item_key, $cart ) {
		...
		if ( ! empty( $cart->removed_cart_contents[ $cart_item_key ][ 'bundled_items' ] ) ) {
			...
			foreach ( $bundled_item_cart_keys as $bundled_item_cart_key ) {
				...
				if ( ! empty( $cart->quote_contents[ $bundled_item_cart_key ] ) ) {

					$remove = $cart->quote_contents[ $bundled_item_cart_key ];
					$cart->removed_cart_contents[ $bundled_item_cart_key ] = $remove;

					unset( $cart->quote_contents[ $bundled_item_cart_key ] );
	...
	public function woo_bundles_cart_item_restored( $cart_item_key, $cart ) {
		...
		if ( ! empty( $cart->quote_contents[ $cart_item_key ][ 'bundled_items' ] ) ) {

			$bundled_item_cart_keys = $cart->quote_contents[ $cart_item_key ][ 'bundled_items' ];
			...
			foreach ( $bundled_item_cart_keys as $bundled_item_cart_key ) {
				...
				if ( ! empty( $cart->removed_cart_contents[ $bundled_item_cart_key ] ) ) {
					...
					$cart->quote_contents[ $bundled_item_cart_key ] = $remove;
