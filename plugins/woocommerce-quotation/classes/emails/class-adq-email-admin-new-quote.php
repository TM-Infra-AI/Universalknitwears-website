...
if ( ! class_exists( 'ADQ_Admin_New_Quote' ) ) :

/**
 * Admin New Quotation email
 *
 * @class 	ADQ_Admin_New_Quote
 * @version     1.0.0
...
class ADQ_Admin_New_Quote extends WC_Email {
	...
	function __construct() {
		...
		$this->id 		= 'admin_new_quote';
		$this->title 		= __( 'Admin request', 'woocommerce-quotation' );
		$this->description	= __( 'Admin Request emails are sent to the admin when a new quote request is send.', 'woocommerce-quotation' );

		$this->heading 		= __( 'Your have a new quote request', 'woocommerce-quotation' );
		$this->subject      	= __( 'Your {site_title} quote request from {order_date} is send', 'woocommerce-quotation' );
    	...
    	$this->form_fields = array(
			'enabled' => array(
				...
				'default' 		=> 'yes'
                        ...
                        'recipient' => array(
				'title' 		=> __( 'Recipient(s)', 'woocommerce-quotation' ),
				'type' 			=> 'text',
				'description' 	=> sprintf( __( 'Enter recipients (comma separated) for this email. Defaults to <code>%s</code>.', 'woocommerce-quotation' ), esc_attr( get_option('admin_email') ) ),
				'placeholder' 	=> '',
				'default' 		=> ''
			),
			'subject' => array(
...
return new ADQ_Admin_New_Quote();
