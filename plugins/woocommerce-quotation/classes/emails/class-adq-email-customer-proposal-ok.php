...
if ( ! class_exists( 'ADQ_Customer_Proposal_Ok' ) ) :

/**
 * Customer Proposal Accepted email
 *
 * @class 	ADQ_Customer_Proposal_Ok
 * @version     1.0.0
 ...
 * @extends     WC_Email
 */
class ADQ_Customer_Proposal_Ok extends WC_Email {
	...
	function __construct() {
		...
		$this->id 		= 'customer_proposal_ok';
		$this->title 		= __( 'Customer proposal ok', 'woocommerce-quotation' );
		$this->description	= __( 'Send email confirming the OK of the proposal.', 'woocommerce-quotation' );

		$this->heading 		= __( 'Proposal accepted', 'woocommerce-quotation' );
		$this->subject      	= __( 'You have accepted our {site_title} proposal from {order_date} is accepted', 'woocommerce-quotation' );

		$this->template_html 	= 'emails/customer-proposal-ok.php';
		$this->template_plain 	= 'emails/plain/customer-proposal-ok.php';
	...
	function trigger( $order_id ) {
		...
		if ( $order_id ) {
			...
			$this->object 		= wc_get_order( $new_order_id );
...
return new ADQ_Customer_Proposal_Ok();
