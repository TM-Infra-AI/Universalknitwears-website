<?php
/**
 * Handle Cookies functions
 *
 * @class 	WC_Quotation_Cookie
 * @version     2.0.0
...
if( !class_exists( 'WC_Quotation_Cookie' ) ) { 
    
define('ADQ_COOKIE_NAME','cookie-quote-request');
define('ADQ_COOKIE_KEY','2e695626e1168587e3ecd4b97df623ba');
define('ADQ_COOKIE_IV','12d1b658f1eb74d589131d3b672ed7aa');
define('ADQ_PRIVATE_IV','8Nc74zf3dsrt43f3');

class WC_Quotation_Cookie
{    
        protected $_separator = 'µ';
	...
	{
                $this->_name = str_replace("=","",base64_encode(ADQ_COOKIE_NAME));
                $this->_blowfish = new BlowfishQuote(ADQ_COOKIE_KEY, ADQ_COOKIE_IV);
                if(isset($_COOKIE[$this->_name]))
                    $this->decrypt($_COOKIE[$this->_name]);              
	}
        ...
        /**
	 * Serialize the array and encrypt it
	 *
	 * @param none
	 ...
	 */
        private function encrypt() {              
                ...
                /*$plaintext = json_encode($this->_variables);
                $ciphertext = $this->_blowfish->encrypt($plaintext);
                
                $ciphertext_base64 = base64_encode($ciphertext).ADQ_PRIVATE_IV;

                return  $ciphertext_base64;*/
		               
		return  base64_encode(json_encode($this->_variables));
        }        
        
        /**
	 * Decrypt the content
	 *
	 * @param $ciphertext_base64 hash required
	 * @return void
	 */
        private function decrypt($ciphertext_base64) {                                         
            
                /*$ciphertext_dec = substr($ciphertext_base64, 0, -strlen(ADQ_PRIVATE_IV));
                $ciphertext_dec = base64_decode($ciphertext_dec);

                $plaintext_dec = $this->_blowfish->decrypt($ciphertext_dec);
                
                $this->_variables = json_decode($plaintext_dec, true); */ 
		$this->_variables = json_decode(base64_decode($ciphertext_base64), true);         
        }	
	...
	public function __unset($key)
	{
        ...
        /**
	 * Magic method wich destructs the class	 
	 */
	...
	/**
	 * Public Function that writes the cookie
	 */
	public function write()
	{
		...
		return @setcookie($this->_name, $content, time() + 1728000, COOKIEPATH, COOKIE_DOMAIN, 0, true);
	}
...
if( !class_exists( 'BlowfishQuote' ) ) { 

define('PS_UNPACK_NATIVE', 1);
define('PS_UNPACK_MODIFIED', 2);

class BlowfishQuote extends Crypt_Blowfish
{
	function encrypt($plaintext)
	{
